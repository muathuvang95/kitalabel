...
//        setup_postdata( $post );
        ...
        echo $this->tagline($post_id);
        echo $this->keywords($post_id);
        if (
            get_query_var('et_is-woocommerce-archive', false) &&
            get_theme_mod( 'et_seo_noindex', 0 )
        ) {
            $this->noidex_product_archives();
        }
        echo $this->image($post_id);
        echo $this->url($post_id);
        echo $this->type();
        echo $this->fb_app_id();
