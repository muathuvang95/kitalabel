...
foreach( $options["fields"] as $key => $field ){
    ...
    if( $field['general']['data_type'] == 'i' ){
        ...
        $tempalte = get_stylesheet_directory() .'/custom-nbdesign/options-builder/label.php'; // MTV
    }
...
if( $group_mode ){
    include( get_stylesheet_directory() .'/custom-nbdesign/options-builder/groups.php' );
}
// foreach( $options["fields"] as $key => $field ){
    ...
    nboApp.controller('optionCtrl', ['$scope', '$timeout', function($scope, $timeout){
        ...
        $scope.check_valid = function( calculate_pm, pro , cur_field_id){
            $timeout(function(){
                ...
                var _groups = $scope.options.groups;
                if(angular.isUndefined($scope.selected_options) || ( angular.isDefined(cur_field_id) && $scope.get_field(cur_field_id).nbd_type == 'area') ) {
                    $scope.selected_options = {};
                }
                if( angular.isDefined($scope.nbd_fields)) {
                    let check_op = true;
                    if( $scope.first_load ) {
                    ...
                    var number_op = 1;
                    angular.forEach( $scope.nbd_fields , function( val , id) {
                        if( val.enable && val.show_in_group ) {              
                            if(number_op < 10 ) {
                                var _number_op = '0' + number_op;
                            }
                            $scope.nbd_fields[id].number_op = _number_op;
                            number_op++;
                        }
                        if( angular.isDefined(cur_field_id) ) {
                            $scope.options_selected_copy[id].enable = val.enable;
                            $scope.options_selected_copy[id].published = val.published;
                            $scope.options_selected_copy[id].show_in_group = val.show_in_group;
                            $scope.options_selected_copy[id].id = id;
                ...
                if( angular.isDefined(_groups[0]) && _groups[0].fields.length > 0 ) {
                    _groups[0].fields.forEach( function( v , k ) {
                        let _fie = $scope.get_field(v);
                        let _nbd_type = _fie.nbd_type;
                        _count_options[_nbd_type] = v;
                    })
                }
                
                if( angular.isDefined(cur_field_id) ) {
                    let _fie = $scope.get_field(cur_field_id);
                    let _nbd_type = _fie.nbd_type;
