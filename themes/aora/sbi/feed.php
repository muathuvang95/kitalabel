...
$aora_atts = json_decode($shortcode_atts, true);

if( !empty($aora_atts['tb-atts']) ) {
	$image_resolution_setting 	= $settings['imageres'];

	if( !empty($aora_atts['data-xlgdesktop']) ) {
		$cols_setting 				= $aora_atts['data-xlgdesktop'];
	} else {
		$cols_setting 				= $aora_atts['data-items'];
	}
	...
	?>
	<div id="sb_instagram" class="sbi sbi_col_<?php echo esc_attr( $cols_setting ); ?>" data-feedid="<?php echo esc_attr( $feed_id ); ?>" data-res="<?php echo esc_attr( $image_resolution_setting ); ?>" data-num="<?php echo esc_attr( $num_setting ); ?>" <?php echo trim($other_atts); ?>>
		...
		<div id="sbi_images" <?php aora_instagram_feed_row_class($aora_atts); ?>>
			<?php
