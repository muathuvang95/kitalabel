...
class MMenu {
  ...
  _initMmenu() {
    ...
    let mmenu_search_form = $('#mmenu-search-form');

    if (mmenu_search_form.length > 0) {
      mmenu_search_form.insertAfter($(mmenu_close));
    }

    let mmenu_footer = $('#mmenu-footer');

    if (mmenu_footer.length > 0) {
      mmenu_footer.insertAfter($('#main-mobile-menu-mmenu-wrapper'));
    }

    let btn_sign_in       = $('.btn-kita-sign-in');
    let login_popup_form  = $('.kita_login_popup_form');
    let login_popup_close = $('.btn-close-popup-login');
    let wrapper_container = $('.wrapper-container');
    let sign_up_now_link  = $('.kita-sign-up-now-link');
    let sign_in_now_link  = $('.kita-sign-in-now-link');

    btn_sign_in.on('click', function(e) {
      e.preventDefault();
      login_popup_form.fadeIn();
      wrapper_container.addClass('kita-popup-show');
    });

    login_popup_close.on('click', function(e) {
      e.preventDefault();
      login_popup_form.fadeOut();
      wrapper_container.removeClass('kita-popup-show');
    });

    sign_up_now_link.on('click', function(e) {
      e.preventDefault();
      $("[aria-controls=register]").trigger('click');
    });

    sign_in_now_link.on('click', function(e) {
      e.preventDefault();
      $("[aria-controls=login]").trigger('click');
    });
