...
class AjaxCart {
  constructor() {
    ...
    if (_this.ajaxCartPosition !== 'popup') {
      $(window).on("resize", () => {
        if (aora_settings.mobile && $(window).width() < 992) {
  ...
  _initAjaxPopupContent(button) {
    let cart_modal = $('#tbay-cart-modal'),
        ...
        cart_notification = aora_settings.popup_cart_noti,
        string = '';
...
class ProductItem {
  initAddButtonQuantity() {
    ...
    input.each(function () {
      ...
      $(`<button class="minus" type="button" value="&nbsp;">${aora_settings.quantity_minus}</button>`).insertBefore($(this));
      $(`<button class="plus" type="button" value="&nbsp;">${aora_settings.quantity_plus}</button>`).insertAfter($(this));
    });
  }

  // custom kitalabel
  kitaCustomInitAddButtonQuantity() {
    let input = $('.nbu-quantity input');
    input.each(function () {
      if ($(this).parent('.box').length) return;
      $(this).wrap('<span class="box"></span>');
      $(`<button class="nb-minus" type="button" value="&nbsp;">${aora_settings.quantity_minus}</button>`).insertBefore($(this));
      $(`<button class="nb-plus" type="button" value="&nbsp;">${aora_settings.quantity_plus}</button>`).insertAfter($(this));
    });
  }
...
class ShopProduct {
  ...
  _SidebarShopMobile() {
    ...
    btn_filter.on("click", function (e) {
      ...
      $("#nbd-chat-app").addClass('hidden'); // custom kitalabel
    });
    btn_close.on("click", function (e) {
      $('.filter-btn-wrapper').removeClass('active');
      $("body").removeClass('filter-mobile-active');
      $("#nbd-chat-app").removeClass('hidden'); // custom kitalabel
    });
  }
...
class SingleProduct {
  ...
  _initFeatureVideo() {
    if (typeof aora_settings === "undefined") return;
    let featured = $(document).find(aora_settings.img_class_container + '.tbay_featured_content');
    if (featured.length === 0) return;
    let featured_index = featured.index(),
        featured_gallery_thumbnail = $(aora_settings.thumbnail_gallery_class_element).get(featured_index);
    $(featured_gallery_thumbnail).addClass('tbay_featured_thumbnail');
  }

  _initMoveTopSingle($resize) {
    let top_single = $('.top-single-product');
    if (top_single.length === 0) return;
    let size = $(window).width();

    if (size < 991) {
      if (top_single.length > 0) {
        top_single.prependTo($(".single-main-content > .row > .information"));
      }
    } else if ($resize) {
      top_single.prependTo($(".singular-shop .single-main-content"));
    }
  }
...
jQuery(document).ready(() => {
  ...
  new AjaxCart(), new WishList(), new Cart(), new Checkout(), new WooCommon(), new LoadMore(), new ModalVideo("#productvideo"), new QuickView(), new DisplayMode(), new ShopProduct(), new AjaxFilter(), new SingleProduct();
});
...
jQuery(window).on('elementor/frontend/init', function () {
  if (typeof aora_settings !== "undefined" && jQuery.isArray(aora_settings.elements_ready.products)) {
