...
if ( ! defined( 'ABSPATH' ) || function_exists('Aora_Elementor_List_Categories_Product') ) {
 ...
 * @since 1.0.0
 */
class Aora_Elementor_List_Categories_Product extends  Aora_Elementor_Carousel_Base{
    /**
     * Get widget name.
     *
    ...
    public function get_title() {
        return esc_html__( 'Aora List Categories Product', 'aora' );
    }
    ...
    protected function register_controls() {
        ...
        $this->start_controls_section(
            'section_general',
            [
                'label' => esc_html__( 'List Categories Product', 'aora' ),
            ]
        );
        $this->add_control(
            'limit',
            [
                'label' => esc_html__('Number of categories', 'aora'),
                'type' => Controls_Manager::NUMBER,
                'description' => esc_html__( 'Number of categories to show ( -1 = all )', 'aora' ),
                'min'  => -1,
                'default' => 6,
            ]
        );
                            ...
                            echo '<div class="cat-content">';
                                ?>
                                    ...
                                    <?php 
                                        if($show_count === 'yes') {
                                            ?>
                                            <span class="count-item"><?php echo trim($count).' '.esc_html__('items', 'aora'); ?></span>
                                            <?php
                                        }
                                    ?>
...
$widgets_manager->register_widget_type(new Aora_Elementor_List_Categories_Product());
