...
if ( ! class_exists( 'Aora_Single_WooCommerce' ) ) :


	class Aora_Single_WooCommerce  {
		...
		public static function getInstance() {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Aora_Single_WooCommerce ) ) {
				self::$instance = new Aora_Single_WooCommerce();
			}
		...
		public function __construct() {
			...
			add_action( 'aora_top_single_product', array( $this, 'share_box_html'), 50 );

    		add_filter( 'aora_woo_class_single_product', array( $this, 'class_single_product'), 10, 1 );

    		add_filter( 'aora_woo_tabs_style_single_product', array( $this, 'get_tabs_style_product'), 10, 1 );
    		...
    		if(!wp_is_mobile() ) {
			    ...
			    add_action( 'aora_sticky_menu_bar_product_summary', 'woocommerce_template_single_title', 5 );
			    add_action( 'aora_sticky_menu_bar_product_summary', 'woocommerce_template_single_rating', 10 );
			    add_action( 'aora_sticky_menu_bar_product_summary', array( $this, 'the_product_single_one_page'), 15 );


			    add_action( 'aora_sticky_menu_bar_product_price_cart', 'woocommerce_template_single_price', 5 );
			    add_action( 'aora_sticky_menu_bar_product_price_cart', array( $this, 'the_sticky_menu_bar_custom_add_to_cart'), 10 );
			}
			...
			add_action( 'aora_woocommerce_before_product_thumbnails', array($this,'get_video_audio_content_first'), 10, 2 );
  			...
  			add_action( 'aora_woo_after_single_rating', array( $this, 'the_total_sales_count'), 10 ); 
		...
		public function add_custom_product_tabs( $tabs ) {
			...
			$post_slug = maybe_unserialize( get_post_meta( $product_id, '_aora_custom_tab' , true ) ); 
			$priority = maybe_unserialize( get_post_meta( $product_id, '_aora_custom_tab_priority' , true ) );
			...
			if( aora_elementor_is_activated() && Elementor\Plugin::$instance->db->is_built_with_elementor( $post->ID ) ) {
				$content = Elementor\Plugin::instance()->frontend->get_builder_content_for_display( $post->ID, aora_get_elementor_css_print_method() );
			} else {
				$content = do_shortcode( $post->post_content );
			} 

			$tabs['aora-customtab'] = array(
		...
		public function setup_support() {
	        ...
	        add_image_size( 'aora_photo_reviews_thumbnail_image', $ptreviews['width'], $ptreviews['height'], $ptreviews['crop'] );
		}
		...
		private function photo_reviews_thumbnail_image() {
            $thumbnail_cropping    	= get_option( 'aora_photo_reviews_thumbnail_image_cropping', 'yes');
            $cropping    			= ($thumbnail_cropping == 'yes') ? true : false;

            return array(
                'width'  => get_option( 'aora_photo_reviews_thumbnail_image_width', 100),
                'height' => get_option( 'aora_photo_reviews_thumbnail_image_height', 100),
                'crop'   => $cropping,
            );		
		}
	        ...
	        <?php if(class_exists('YITH_WCWL') || class_exists('YITH_Woocompare')){ ?>
	            ...
	            <?php if(class_exists('YITH_WCWL')) { ?> 
	                <div class="tbay-wishlist">
	                   <?php aora_the_yith_wishlist(); ?>
	                </div>  
	            <?php } ?>
	            <?php if(class_exists('YITH_Woocompare')){ ?>
	                <div class="tbay-compare">
	                    <?php aora_the_yith_compare($product->get_id()); ?>
	                </div>
	            <?php } ?>
	            </div>
	        <?php } ?>
		...
		public function buy_now_html() {
			if( aora_catalog_mode_active() ) return;
 
			global $product;
	        if ( ! intval( aora_tbay_get_config('enable_buy_now', false) ) ) {
	            return; 
	        }
	        ...
	        echo sprintf( '<button class="'. $class .'">%s</button>', esc_html__('Buy Now', 'aora') );
	        echo '<input type="hidden" value="0" name="aora_buy_now" />';
		}

		public function buy_now_redirect( $url ) {

	 		if ( ! isset( $_REQUEST['aora_buy_now'] ) || $_REQUEST['aora_buy_now'] == false ) {
	            return $url; 
	        }
	        ...
	        $redirect = aora_tbay_get_config('redirect_buy_now', 'cart') ;
		...
		public function the_list_images_review() {
	        ...
	        if (is_array($comments) || is_object($comments)) {
	            ...
	            $outputs .= '<h4>'. esc_html__('Images from customers:', 'aora') .'</h4>';
	            $outputs .= '<ul>';
		...
		public function share_box_html() {
			if( !aora_tbay_get_config('enable_code_share',false) || !aora_tbay_get_config('enable_product_social_share', false) ) return;
		...
		public function class_single_product( $styles ) {
	        ...
	        $active = aora_tbay_get_config('enable_size_guide', false);
			...
			$cart_style = aora_get_mobile_form_cart_style();
		...
		public function get_tabs_style_product( $tabs_layout ) {
			if ( is_singular( 'product' ) ) {
	          $tabs_style       = aora_tbay_get_config('style_single_tabs_style', 'tabs');
		...
		public function the_product_single_time_countdown() {
	        ...
	        $style_countdown   = aora_tbay_get_config('enable_product_countdown',false);
	        ...
	        <?php if ( $time_sale ): ?>
	            <div class="tbay-time-wrapper">
	              <div class="time tbay-time">
	                  <div class="title"><?php printf( __( 'Deal will end soon! Only %s left', 'aora' ), $stock_quantity ); ?></div>
		...
		public function the_sticky_menu_bar() {
			global $post, $product;
		      ...
		      $menu_bar   =  aora_get_product_menu_bar();
		        ...
		        <div class="container">
		          <div class="row">
		            <div class="menu-bar-left col-lg-7">
		                <div class="media">
		                  ...
		                  <div class="media-body">
		                    <?php 
		                      do_action( 'aora_sticky_menu_bar_product_summary' );
		                    ?>
		                  </div>
		                </div>
		            </div>
		            <div class="menu-bar-right product col-lg-5">
		                <?php 
		                  do_action( 'aora_sticky_menu_bar_product_price_cart');
		                ?>
		            </div>
		          </div>
		        </div>
		...
		public function the_product_single_one_page() {
			$menu_bar   =  apply_filters( 'aora_woo_product_menu_bar', 10, 2 );
		...
		public function before_add_to_cart_form() {
			...
			global $product;
	      	...
	      	if( intval( aora_tbay_get_config('enable_buy_now', false) ) && $product->get_type() !== 'external' ) {
	      		$class .= ' has-buy-now';
	      	}
		...
		public function get_related_products_args( $args ) {
		    $args['posts_per_page'] = aora_tbay_get_config('number_product_releated', 5); // 4 related products
		...
		public function get_featured_image_id( $product, $video_id, $host ) {
			...
			$thumbnail_id = $product->get_meta( '_aora_video_image_url' );
			return $thumbnail_id;
		}
		...
		public function get_featured_video_args( $product ) {
			...
			if ( ! empty( $video_url ) ) {
				...
				list( $host, $video_id ) = explode( ':', aora_video_type_by_url( $video_url ) );
		...
		public function get_video_audio_content_last( ) {
			if( aora_tbay_get_config('video_position', 'last') === 'first' ) return;
		...
		public function get_video_audio_content_first() {
			...
			if( aora_tbay_get_config('video_position', 'last') === 'last' ) return;
		...
		public function the_product_size_guide() {
			$active = aora_tbay_get_config('enable_size_guide', false);
		    $icon = aora_tbay_get_config('size_guide_icon', '');
		    ...
		    if ( !$active || !metadata_exists( 'post', $post->ID, '_product_size_guide_image' ) )  return;
		      ...
		      $title = aora_tbay_get_config('size_guide_title', false);
		    ...
		    <div class="reset-button">
		    	<a class="reset_variations" href="#"><?php esc_html_e( 'Clear all', 'aora' ) ?></a>
			</div>
		    <?php
		}
		...
		public function remove_review_tab( $tabs ) {
	 		if ( !aora_tbay_get_config('enable_product_review_tab', true) && isset($tabs['reviews']) ) {
	            unset( $tabs['reviews'] ); 
	        }
	        return $tabs;
		}
		...
		public function enable_zoom_image() {
	     	$active = aora_tbay_get_config('enable_zoom_image', true);
		...
		public function html_before_add_to_cart_button() {
			$content = aora_tbay_get_config('html_before_add_to_cart_btn');
     		echo trim($content);
		}

		public function html_after_add_to_cart_button() {
			$content = aora_tbay_get_config('html_after_add_to_cart_btn');
	      	echo trim($content); 
		}
		...
		public function the_total_sales_count() {
		    ...
		    if( !intval( aora_tbay_get_config('enable_total_sales', true) ) || $product->get_type() == 'external' ) return;
		...
		public function get_photo_reviews_thumbnail_size($image_src, $image_post_id) {
			$img_src     = wp_get_attachment_image_src($image_post_id, 'aora_photo_reviews_thumbnail_image');
		...
		public function photo_reviews_reduce_array( $reduce ) {
			array_push($reduce,'aora_photo_reviews_thumbnail_image');
			return $reduce;
		}
		...
		public function mobile_add_add_to_cart_button_content() {
			...
			if( aora_get_mobile_form_cart_style() === 'default' ) return;
		...
		public function mobile_add_btn_after_add_to_cart_form() {
			...
			$class = '';
			if( aora_tbay_get_config('enable_buy_now', false) ) {
				$class .= ' has-buy-now';
			}  
			...
			<div class="mobile-btn-cart-click <?php echo esc_attr($class); ?>">
				<div id="tbay-click-addtocart"><?php esc_html_e('Add to cart', 'aora') ?></div>
				<?php if( aora_tbay_get_config('enable_buy_now', false) ) : ?>
					<div id="tbay-click-buy-now"><?php esc_html_e('Buy Now', 'aora') ?></div>
				<?php endif; ?> 
			</div>
			<?php
		}
		...
		public function mobile_add_before_add_to_cart_form( ) {
			...
			foreach ( $attributes as $key => $value ) {
				...
				if( isset($selected_attributes[$key]) && !empty($selected_attributes[$key]) )  {
					...
					$default = esc_html__('Choose an option ', 'aora');
				}
...
if ( !function_exists('aora_single_wooCommerce') ) {
	function aora_single_wooCommerce() { 
		return Aora_Single_WooCommerce::getInstance();
	}
	aora_single_wooCommerce();
}