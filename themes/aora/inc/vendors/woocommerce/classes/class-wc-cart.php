...
if ( ! class_exists( 'Aora_Cart' ) ) :
	...
	class Aora_Cart  {
		...
		public static function getInstance() {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Aora_Cart ) ) {
				self::$instance = new Aora_Cart();
			}
		...
		public function __construct() {
			...
			/*Cart modal*/
			add_action( 'wp_ajax_aora_add_to_cart_product', array( $this, 'woocommerce_cart_modal'), 10 );
			add_action( 'wp_ajax_nopriv_aora_add_to_cart_product', array( $this, 'woocommerce_cart_modal'), 10 );
			add_action( 'wp_footer', array( $this, 'add_to_cart_modal_html'), 20 );

			add_filter( 'aora_cart_position', array( $this, 'woocommerce_cart_position'), 10 ,1 );  
			...
			add_filter( 'aora_show_cart_mobile', array( $this, 'show_cart_mobile'), 10, 1 );
		...
		public function woocommerce_cart_position() {
	        ...
	        $position = aora_tbay_get_config('woo_mini_cart_position', 'popup');

	        $position = ( isset($_GET['ajax_cart']) ) ? $_GET['ajax_cart'] : $position;

	        $position =  (!in_array($position, $position_array)) ? aora_tbay_get_config('woo_mini_cart_position', 'popup') : $position;
		...
		public function add_to_cart_message_html_mobile( $message ) {
			if ( isset( $_REQUEST['aora_buy_now'] ) && $_REQUEST['aora_buy_now'] == true ) {
	            return __return_empty_string();
	        }

	        if ( wp_is_mobile() && ! intval( aora_tbay_get_config('enable_buy_now', false) ) ) {
	            return __return_empty_string();     
	        } else {
	            return $message;
	        }
		...
		public function show_cart_mobile() {
			$active = aora_tbay_get_config('enable_add_cart_mobile', false);
		...
		public function body_classes_show_cart_mobile( $classes ) {
	 		$class = '';
	        $active = apply_filters( 'aora_show_cart_mobile', 10,2 );
...
if ( !function_exists('aora_cart') ) {
	function aora_cart() { 
		return Aora_Cart::getInstance();
	}
	aora_cart();
}