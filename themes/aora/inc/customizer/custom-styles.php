...
//convert hex to rgb
if ( !function_exists ('aora_tbay_getbowtied_hex2rgb') ) {
	function aora_tbay_getbowtied_hex2rgb($hex) {
		$hex = str_replace("#", "", $hex);
...
if ( !function_exists ('aora_tbay_color_lightens_darkens') ) {
	/**
	 ...
	 */
	function aora_tbay_color_lightens_darkens( $hex, $percent ) {
...
if ( !function_exists ('aora_tbay_default_theme_primary_color') ) {
	function aora_tbay_default_theme_primary_color() {
		$theme_variable = array();

		$theme_variable['main_color'] 			= '#fcb800';

		$theme_variable['main_color_second'] 	= '#023047';

		return apply_filters( 'aora_get_default_theme_color', $theme_variable);
	}
}

if ( !function_exists ('aora_tbay_custom_styles') ) {
	function aora_tbay_custom_styles() {
		global $reduxConfig;	

		$output = $reduxConfig->output;

		$main_color  		= $main_bg_color =  $main_border_color  = aora_tbay_get_config('main_color');

		$main_color_second      	= aora_tbay_get_config('main_color_second');

		$logo_img_width        		= aora_tbay_get_config( 'logo_img_width' );
		$logo_padding        		= aora_tbay_get_config( 'logo_padding' );	

		$logo_img_width_mobile 		= aora_tbay_get_config( 'logo_img_width_mobile' );
		$logo_mobile_padding 		= aora_tbay_get_config( 'logo_mobile_padding' );

		$custom_css 			= aora_tbay_get_config( 'custom_css' );
		$css_desktop 			= aora_tbay_get_config( 'css_desktop' );
		$css_tablet 			= aora_tbay_get_config( 'css_tablet' );
		$css_wide_mobile 		= aora_tbay_get_config( 'css_wide_mobile' );
		$css_mobile         	= aora_tbay_get_config( 'css_mobile' );

		$show_typography        = (bool) aora_tbay_get_config( 'show_typography', false );
		...
		<?php if( $show_typography ) : ?>	
			...
			/* Main Font */
			<?php
				$font_source 			= aora_tbay_get_config('font_source');
				$main_font 				= aora_tbay_get_config('main_font');
				$main_google_font_face = aora_tbay_get_config('main_google_font_face');
				$main_custom_font_face = aora_tbay_get_config('main_custom_font_face');
		...
		<?php endif; ?>
			...
			<?php if ( $main_color != "" ) : ?>
				...
				<?php echo trim($output['background_hover']); ?> {
					background: <?php echo esc_html( aora_tbay_color_lightens_darkens( $main_bg_color, -0.1) ); ?>;
				}
				<?php endif; ?>

			<?php endif; ?> 

			<?php if( $main_color_second != "" ) : ?>
				/*color*/
				<?php if( isset($output['main_color_second_hover']['color']) && !empty($output['main_color_second_hover']['color']) ) : ?>
				<?php echo trim($output['main_color_second_hover']['color']); ?> {
					color: <?php echo esc_html( aora_tbay_color_lightens_darkens( $main_color_second, -0.1) ); ?>;
				}
				<?php endif; ?>
				/*background*/
				<?php if( isset($output['main_color_second_hover']['background-color']) && !empty($output['main_color_second_hover']['background-color']) ) : ?>
				<?php echo trim($output['main_color_second_hover']['background-color']); ?> {
					background: <?php echo esc_html( aora_tbay_color_lightens_darkens( $main_color_second, -0.1) ); ?>;
				}
				<?php endif; ?>
				/*border*/
				<?php if( isset($output['main_color_second_hover']['border-color']) && !empty($output['main_color_second_hover']['border-color']) ) : ?>
				<?php echo trim($output['main_color_second_hover']['border-color']); ?> {
					border-color: <?php echo esc_html( aora_tbay_color_lightens_darkens( $main_color_second, -0.1) ); ?>;
				} 
				<?php endif; ?>
			<?php endif; ?>
	...
	<?php
		...
		wp_enqueue_style( 'aora-style', AORA_THEME_DIR . '/style.css', array(), '1.0' );

		wp_add_inline_style( 'aora-style', $custom_css );
	...
	add_action( 'wp_enqueue_scripts', 'aora_tbay_custom_styles', 200 ); 
}