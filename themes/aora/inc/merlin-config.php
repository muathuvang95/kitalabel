...
require_once( get_parent_theme_file_path( AORA_INC . '/merlin-data/elementor.php') );

class Aora_Merlin_Config {
	...
	private function init() {
		$wizard = new Merlin(
			...
			$strings = array(
				'admin-menu'          => esc_html__( 'Theme Setup', 'aora' ),

				/* translators: 1: Title Tag 2: Theme Name 3: Closing Title Tag */
				'title%s%s%s%s'       => esc_html__( '%1$s%2$s Themes &lsaquo; Theme Setup: %3$s%4$s', 'aora' ),
				'return-to-dashboard' => esc_html__( 'Return to the dashboard', 'aora' ),
				'ignore'              => esc_html__( 'Disable this wizard', 'aora' ),

				'btn-skip'                 => esc_html__( 'Skip', 'aora' ),
				'btn-next'                 => esc_html__( 'Next', 'aora' ),
				'btn-start'                => esc_html__( 'Start', 'aora' ),
				'btn-no'                   => esc_html__( 'Cancel', 'aora' ),
				'btn-plugins-install'      => esc_html__( 'Install', 'aora' ),
				'btn-child-install'        => esc_html__( 'Install', 'aora' ),
				'btn-content-install'      => esc_html__( 'Install', 'aora' ),
				'btn-import'               => esc_html__( 'Import', 'aora' ),
				'btn-license-activate'     => esc_html__( 'Activate', 'aora' ),
				'btn-license-skip'         => esc_html__( 'Later', 'aora' ),

				/* translators: Theme Name */
				'license-header%s'         => esc_html__( 'Activate %s', 'aora' ),
				/* translators: Theme Name */
				'license-header-success%s' => esc_html__( '%s is Activated', 'aora' ),
				/* translators: Theme Name */
				'license%s'                => esc_html__( 'Enter your license key to enable remote updates and theme support.', 'aora' ),
				'license-label'            => esc_html__( 'License key', 'aora' ),
				'license-success%s'        => esc_html__( 'The theme is already registered, so you can go to the next step!', 'aora' ),
				'license-json-success%s'   => esc_html__( 'Your theme is activated! Remote updates and theme support are enabled.', 'aora' ),
				'license-tooltip'          => esc_html__( 'Need help?', 'aora' ),

				/* translators: Theme Name */
				'welcome-header%s'         => esc_html__( 'Welcome to %s', 'aora' ),
				'welcome-header-success%s' => esc_html__( 'Hi. Welcome back', 'aora' ),
				'welcome%s'                => esc_html__( 'This wizard will set up your theme, install plugins, and import content. It is optional & should take only a few minutes.', 'aora' ),
				'welcome-success%s'        => esc_html__( 'You may have already run this theme setup wizard. If you would like to proceed anyway, click on the "Start" button below.', 'aora' ),

				'child-header'         => esc_html__( 'Install Child Theme', 'aora' ),
				'child-header-success' => esc_html__( 'You\'re good to go!', 'aora' ),
				'child'                => esc_html__( 'Let\'s build & activate a child theme so you may easily make theme changes.', 'aora' ),
				'child-success%s'      => esc_html__( 'Your child theme has already been installed and is now activated, if it wasn\'t already.', 'aora' ),
				'child-action-link'    => esc_html__( 'Learn about child themes', 'aora' ),
				'child-json-success%s' => esc_html__( 'Awesome. Your child theme has already been installed and is now activated.', 'aora' ),
				'child-json-already%s' => esc_html__( 'Awesome. Your child theme has been created and is now activated.', 'aora' ),

				'plugins-header'         => esc_html__( 'Install Plugins', 'aora' ),
				'plugins-header-success' => esc_html__( 'You\'re up to speed!', 'aora' ),
				'plugins'                => esc_html__( 'Let\'s install some essential WordPress plugins to get your site up to speed.', 'aora' ),
				'plugins-success%s'      => esc_html__( 'The required WordPress plugins are all installed and up to date. Press "Next" to continue the setup wizard.', 'aora' ),
				'plugins-action-link'    => esc_html__( 'Advanced', 'aora' ),

				'import-header'      => esc_html__( 'Import Content', 'aora' ),
				'import'             => esc_html__( 'Let\'s import content to your website, to help you get familiar with the theme.', 'aora' ),
				'import-action-link' => esc_html__( 'Advanced', 'aora' ),

				'ready-header'      => esc_html__( 'All done. Have fun!', 'aora' ),

				/* translators: Theme Author */
				'ready%s'           => esc_html__( 'Your theme has been all set up. Enjoy your new theme by %s.', 'aora' ),
				'ready-action-link' => esc_html__( 'Extras', 'aora' ),
				'ready-big-button'  => esc_html__( 'View your website', 'aora' ),
				'ready-link-1'      => sprintf( '<a href="%1$s" target="_blank">%2$s</a>', 'https://tickets.thembay.com/', esc_html__( 'Ticket System', 'aora' ) ),
				'ready-link-2'      => sprintf( '<a href="%1$s">%2$s</a>', 'https://docs.thembay.com/aora/', esc_html__( 'Documentation', 'aora' ) ),
				'ready-link-3'      => sprintf( '<a href="%1$s">%2$s</a>', 'https://www.youtube.com/c/thembay/', esc_html__( 'Video Tutorials', 'aora' ) ),
				'ready-link-4'      => sprintf( '<a href="%1$s">%2$s</a>', 'https://forums.thembay.com/', esc_html__( 'Forums', 'aora' ) ),
			)
		);
	}
...
- - - - - - - - - - - - - - - - - - - - */";
	...
	public function after_import_setup( $selected_import ) {
		...
		$check_oneclick  = get_option( 'aora_check_oneclick', [] );
		...
		if ( ! isset( $check_oneclick[ $selected_import['home'] ] ) || apply_filters( 'aora_reset_import_rev_sliders', false ) ) {
			$check_oneclick[ $selected_import['home'] ] = true;
			$this->import_revslider( $selected_import['rev_sliders'] );
			update_option( 'aora_check_oneclick', $check_oneclick );
		}
	...
	private function update_option_woof() {
		...
		$settings['by_price']['title_text'] = esc_html__('Price', 'aora');

		/**Categories**/
		$settings['tax']['product_cat'] = '1';
		$settings['show_title_label']['product_cat'] = '1';
		$settings['custom_tax_label']['product_cat'] = esc_html__('Categories', 'aora');

		/**Size**/
		$settings['tax']['pa_size'] = '1';
		$settings['show_title_label']['pa_size'] = '1';
		$settings['custom_tax_label']['pa_size'] = esc_html__('Product Size', 'aora');

		/**Color**/
		$settings['tax']['pa_color'] = '1';
		$settings['show_title_label']['pa_color'] = '1';
		$settings['custom_tax_label']['pa_color'] = esc_html__('Product Color', 'aora');

		/**Tag**/
		$settings['tax']['product_tag'] = '1';
		$settings['show_title_label']['product_tag'] = '1';
		$settings['custom_tax_label']['product_tag'] = esc_html__('Product Tags', 'aora');

		/**Brand**/
		if( class_exists( 'YITH_WCBR' ) ) {
			$settings['tax']['yith_product_brand'] = '1';
			$settings['show_title_label']['yith_product_brand'] = '1';
			$settings['custom_tax_label']['yith_product_brand'] = esc_html__('Brands', 'aora');
		}	
	...
	public function import_files(){
		$data = new Aora_Merlin_Elementor;

		$prefix = $this->import_files_type_demo();

		if( is_rtl() ) return $data->import_files_rtl();

		switch ($prefix) {
			case 'dokan':
				return $data->import_files_dokan();
				break;
			...
			default:
				return $data->import_files_default();
				break;
		}
	}
}

return new Aora_Merlin_Config();
