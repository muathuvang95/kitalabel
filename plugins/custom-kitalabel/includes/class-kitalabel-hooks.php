...
if (!class_exists('Kitalabel_Custom_Hooks')) {
    ...
    class Kitalabel_Custom_Hooks {
        ...
        public function kitalabel_zip_files_and_download( $file_names, $archive_file_name, $nameZip, $option_name = array(), $download = true ){
        ...
        public function kitalabel_download_all($order, $order_id) {
            ...
            foreach( $products AS $order_item_id => $product ){
                if( wc_get_order_item_meta( $order_item_id, '_nbd' ) || wc_get_order_item_meta( $order_item_id, '_nbu' ) ){
                    ...
                    if( $nbd_item_key ){
                        ...
                        if( count( $list_pdf ) > 0 ){
                            foreach( $list_pdf as $key => $file_pdf ){
                                $zip_files[] = $file_pdf;
                            }
                        }
                    }
            ...
            if( !count( $zip_files ) ){
                ...
                $this->kitalabel_zip_files_and_download( $zip_files, $pathZip, $nameZip );
            }
        }
        ...
        public function kitalbel_convert_pdf_item() {
            ...
            $result = array(
                'created' => count($list_pdf) > 0 ? true : false,
            );
        ...
        public function kitalabel_download_pdf_all($order, $order_id) {
            $this->kitalabel_download_all($order, $order_id);
            if( isset($_POST['kitalabel-download-all']) && $_POST['kitalabel-download-all'] == 'download-all') {
                var_dump($_POST['kitalabel-download-all']);
                $this->kitalabel_download_all($order, $order_id);
            }
            ?>
                    ...
                    <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
                        <div class="modal-content">
                            ...
                            <div class="modal-footer">
                                <input type="hidden" name="kitalabel-download-all" value="download-all">
                                <button type="button" class="btn btn-secondary kitalabel-convert-pdf-all">Convert all</button>
                                <button type="submit" class="btn btn-primary kitalabel-download-pdf-all" <?php echo esc_attr(!$can_download_all ? 'disabled': ''); ?>>Download all</button>
                            </div>
                        </div>
                    </div>
                </form>
