<div class="nbd-stages" id="nbd-stages" nbd-drop-zone="uploadFile(files)">
    <div class="stages-inner">
        <div class="stage" ng-repeat="(stageIndex, stage) in stages" id='stage-container-{{$index}}' ng-mousedown="onClickStage($event)" ng-class="{'_nbd_hidden':$index > 0}" >
            ...
            <div ng-if="settings.showDimensions && !settings.is_mobile && !paddingOptions[stageIndex].check" class="nbd-dimension x-dimension" ng-style="{'left': calcRulerPaddingLeft(stage.config.cwidth * stage.states.scaleRange[stage.states.currentScaleIndex].ratio), 'width': calcStyle(stage.config.cwidth * stage.states.scaleRange[stage.states.currentScaleIndex].ratio + 1)}"><span>{{settings.product_data.product[$index].product_width + ' ' + settings.nbdesigner_dimensions_unit}}</span></div>
            <div ng-if="settings.showDimensions && !settings.is_mobile && !paddingOptions[stageIndex].check" class="nbd-dimension y-dimension" ng-style="{'top': calcRulerPaddingTop(stage.config), 'height': calcStyle(stage.config.cheight * stage.states.scaleRange[stage.states.currentScaleIndex].ratio + 1)}"><span class="dimension-number-wrap"><span class="dimension-number">{{settings.product_data.product[$index].product_height + ' ' + settings.nbdesigner_dimensions_unit}}</span></span></div>

            <div ng-if="settings.showDimensions && !settings.is_mobile && paddingOptions[stageIndex].check" class="nbd-dimension x-dimension" ng-style="{'left': calcRulerPaddingLeft(stage.config.cwidth * stage.states.scaleRange[stage.states.currentScaleIndex].ratio), 'width': calcStyle( paddingOptions[stageIndex].width * (stage.config.cwidth * stage.states.scaleRange[stage.states.currentScaleIndex].ratio + 1)/(settings.product_data.product[$index].product_width) ), 'margin-left':calcStyle( paddingOptions[stageIndex].padding * (stage.config.cwidth * stage.states.scaleRange[stage.states.currentScaleIndex].ratio + 1)/(settings.product_data.product[$index].product_width) )}"><span>{{paddingOptions[stageIndex].width + ' ' + settings.nbdesigner_dimensions_unit}}</span></div>
            <div ng-if="settings.showDimensions && !settings.is_mobile && paddingOptions[stageIndex].check" class="nbd-dimension y-dimension" ng-style="{'top': calcRulerPaddingTop(stage.config), 'height': calcStyle( paddingOptions[stageIndex].height*(stage.config.cheight * stage.states.scaleRange[stage.states.currentScaleIndex].ratio + 1)/(settings.product_data.product[$index].product_height) ), 'margin-top':calcStyle( paddingOptions[stageIndex].padding * (stage.config.cheight * stage.states.scaleRange[stage.states.currentScaleIndex].ratio + 1)/(settings.product_data.product[$index].product_height) )}"><span class="dimension-number-wrap"><span class="dimension-number">{{paddingOptions[stageIndex].height + ' ' + settings.nbdesigner_dimensions_unit}}</span></span></div>
    ...
    <div class="nbd-stages-nav" ng-show="!settings.is_mobile">
