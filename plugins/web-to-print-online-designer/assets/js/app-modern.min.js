...
nbdApp.controller('designCtrl', ['$scope', 'FabricWindow', '$window', 'NBDDataFactory', 'filterFontFilter', 'filterArtFilter', '$timeout', '$http', '$document', '$interval',
    ...
    $scope.initSettings = function(){
        ...
        if(NBDESIGNCONFIG.show_nbo_option){
            // $timeout(function(){
                $scope.getPrintingOptions(false); // custom kitalabel not show options when go to editor
            // }, 100);
        }
    ...
    $scope.applyOptions = function(){
        ...
        if(angular.isDefined(nbd_window.nbOption) && angular.isDefined(nbd_window.nbOption.nbd_fields)) {
            $scope.printingOptions = new Blob([JSON.stringify(nbd_window.nbOption.nbd_fields)], {type: "application/json"});
        }
        $scope.changePrintingOptions();
    };
            ...
            if( angular.isDefined( data.page ) ){
                ...
                if( angular.isDefined( data.page.list_page ) ){
                    ...
                    if( dst_number_side < number_side ){
                        $scope.settings.product_data.product.splice(dst_number_side, number_side - dst_number_side);
                        ...
                        //     insert_side_index++;
                        // }
                        // custom kita label: Thay đổi tên thành Design + thứ tự side trỏng editor design
                        for(i = number_side; i < dst_number_side; i++){
                            var temp = {};
                            angular.copy($scope.settings.product_data.product[src_side_index], temp);
                            temp.orientation_name = 'Design ' + (i+1);
                            $scope.settings.product_data.product.splice(i, 0, temp);
                        }
                    }
                    $scope.stored_product = [];
                    angular.copy($scope.settings.product_data.product, $scope.stored_product);
...
/*                 angular.forEach($scope.settings.product_data.product, function(side, key){
            ...
            if( angular.isDefined( data.padding ) ){
                ...
                angular.forEach($scope.settings.product_data.product, function(side, key){
                    ...
                    let paddingOption = {
                        check : true,
                        width : parseFloat(side.product_width),
                        height : parseFloat(side.product_height),
                        padding : padding
                    };
                    $scope.paddingOptions[key] = paddingOption;
...
/*                 if( angular.isDefined( data.area ) ){
    ...
    $scope.loadedPrintingOptionsFirst = false;
    $scope.widthoutPrintingOptions = false;

    // custom kitalabel not show options when go to editor ( add "click")
    $scope.getPrintingOptions = function(click = true){
        if( $scope.settings.nbdesigner_display_product_option == '1' && click ){
            jQuery('.nbd-popup.popup-nbo-options').nbShowPopup();
        }
        if($scope.loadedPrintingOptionsFirst) {
            return;
        }
        $scope.loadedPrintingOptionsFirst = true;
    ...
    $scope.saveData = function(type){
        ...
        function _saveData(){
            ...
            if(angular.isDefined(nbd_window.nbOption) && angular.isDefined($scope.printingOptions)) {
                dataObj.printing_options = new Blob([$scope.printingOptions], {type: "application/json"}); // custom kitalabel
            }
            _.each($scope.stages, function(stage, index){
                // custom design name
                if( angular.isDefined( $scope.resource.config.product[index] )) {
                    $scope.resource.config.product[index].orientation_name = stage.config.name;
                }
                //end
            ...
            switch(type){
                ...
                default:
                    ...
                    $timeout(function(){
                        ...
                        dataObj['options-update'] = jQuery('#nbo-options-wrap .quick-view form').serialize(); //custom Kitalabel
                        NBDDataFactory.get(action, dataObj, function(data){
                            data = JSON.parse(data);
                            if(data.flag == 'success'){
                                if( type == 'save_draft' ){
                                ...
                                if( type == 'saveforlater' ){
                                    ...
                                    if(NBDESIGNCONFIG.show_nbo_option == "1" && ( NBDESIGNCONFIG.task == 'new' || NBDESIGNCONFIG.task2 == 'update' ) ){
                                        ...
                                        if(NBDESIGNCONFIG['cart_item_key'] != '' && NBDESIGNCONFIG['task'] == 'new' && NBDESIGNCONFIG['task2'] == 'update'){
                                            window.location = NBDESIGNCONFIG['redirect_url'];
                                            return;
                                        }
                                        jQuery('.variations_form, form.cart').submit();
                                    ...
                                    // custom kitalabel
                                    var type_page = nbd_window.jQuery('input[name="nb-custom-design-page"]').val();
                                    if( type_page == 'custom_design_page' ) {
    ...
    $scope.onObjectAdded = function(id, options){
        ...
        if( $scope.contextAddLayers == 'normal' && !$scope.resource.drawMode.status ){
            ...
            // $scope.toggleTip(); // custom Kitalabel Ẩn Tip
        }
    ...
    $scope.setHistory = function(stage_id, undo, redo, hasJson){
        ...
        if ( undo ) {
            ...
            if( typeof _canvas.toJSON == 'function' ){
                ...
                var a = _stage.undos[_stage.undos.length - 1];
                if(!angular.equals(a, json)){
...
/*         var _canvas = this.stages[stage_id]['canvas'];
    ...
    $scope.setLayerAttribute = function(type, value, item_index, layer_index){
        ...
        if(angular.isDefined(item_index)){
            // Fix lỗi khi item index bị sai
            if(typeof $scope.stages[$scope.currentStage].layers[layer_index] == 'undefined') {
                angular.forEach($scope.stages[$scope.currentStage].layers, function(value, key) {
                    $scope.stages[$scope.currentStage].layers[key].index = key;
                });
                layer_index -=1;
            }
    ...
    return {
        get : function(action, data, callback, progressCallback) {
            ...
            angular.forEach(data, function (value, key) {
                var keepDefault = ['file', 'design', 'config', 'product', 'upload', 'used_font', 'option', 'preview', 'template_thumb', 'printing_options'];
