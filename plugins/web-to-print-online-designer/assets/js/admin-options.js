            ...
            if( angular.isUndefined( ftype ) ){
                ...
                switch(type){
                    ...
                    case 'area':
                        ...
                        field.general.attributes.options[2] = {};
                        field.general.attributes.options[3] = {};
                        field.general.attributes.options[4] = {};
                        angular.copy(field.general.attributes.options[0], field.general.attributes.options[1]); 
                        angular.copy(field.general.attributes.options[0], field.general.attributes.options[2]); 
                        // Custom Kitalabel
                        angular.copy(field.general.attributes.options[0], field.general.attributes.options[3]); 
                        angular.copy(field.general.attributes.options[0], field.general.attributes.options[4]); 
                        field.general.attributes.options[0].name = 'Circle';
                        field.general.attributes.options[1].name = 'Rectangle';
                        field.general.attributes.options[2].name = 'Oval';
                        field.general.attributes.options[3].name = 'Square';
                        field.general.attributes.options[4].name = 'Custom';
                        field.general.attributes.add_att = false;
                        field.general.attributes.remove_att = false;
                        break;
                    case 'shape':
...
//        }
    ...
    $scope.init = function( options ){
        ...
        $scope.update_options_combination($scope.options);
    ...
    //custom kitalabel
    $scope.update_options_combination = function( options , action = '' ) {
    	if( options.groups.length > 0 ) {
    		...
    		$scope.options_areas = [];
    		$scope.options_sizes = [];
    		$scope.options_materials = [];

    		var options_combination = [];
    		angular.forEach(fields, function( field, key ){
    			if( field.nbd_type == 'area' ) {
    				op_areas.push(field);
    			}
    			if( field.nbd_type == 'size' ) {
    				op_sizes.push(field);
    			}
    			if( field.nbd_type == 'color' ) {
    				op_materials.push(field);
    			}
    		})
    		if( op_areas.length > 0 ) {
    			var has_square = 0;
    			var has_oval = 0;
    			angular.forEach(op_areas[0].general.attributes.options , function( op_area , k1 ){
    				var _op_sizes = [];
					angular.forEach( op_sizes , function( op_size , _k1 ) {
    					if( op_size.conditional.depend[0].id == op_areas[0].id && op_size.conditional.depend[0].val == k1 ) {
    						_op_sizes = op_size.general.attributes.options;
    					}
    					if(_op_sizes.length == 0) {
    						_op_sizes = op_sizes[0].general.attributes.options;
    					}
    				})
    				$scope.options_sizes[k1] =  _op_sizes;
    				var custom = 1;
    				var _op_area = [];
        			_op_area = angular.copy( op_area, _op_area );
    				if( ( op_area.name == 'Square' || op_area.name == 'Circle') ) {
    					_op_area.name = 'Square + Circle';
    					if( has_square == 0 && _op_area.coming_soon != 'on') {
    						$scope.options_areas.push(_op_area);
    						has_square = 1;
    					} 				
    					custom = 0;
    				}
    				if( ( op_area.name == 'Rectangle' || op_area.name == 'Oval') ) {
    					_op_area.name = 'Rectangle + Oval';
    					if( has_oval == 0 && _op_area.coming_soon != 'on' ) {
    						$scope.options_areas.push(_op_area);
    						has_oval = 1;
    					}
    					custom = 0;
    		...
    		if( op_materials.length > 0 ) {
    			$scope.options_materials =op_materials[0].general.attributes.options;
    		}
    	}
    	// $timeout(function() {
     //        $scope.update_no_combination(options);
     //    });
    }
    $scope.update_no_combination = function(show) {
    	var options = $scope.options;
    	if( show ) {
    		var index = 1;
	    	angular.forEach( options.combination.options , function( area , k1 ) {
				angular.forEach( area , function( size , k2 ) {
					angular.forEach( size , function( material , k3 ) {
						if($scope.options.combination.options[k1][k2][k3].coming_soon != 'on') {
		    				$scope.options.combination.options[k1][k2][k3].index = index;
		    				$scope.options.combination.options[k1][k2][k3]['show'] = false;
		    				if(angular.isUndefined( $scope.options.combination.options[k1][k2][k3]['qty_breaks'] )) {
		    					$scope.options.combination.options[k1][k2][k3]['qty_breaks'] = [];
			    				$scope.options.combination.options[k1][k2][k3]['qty_breaks'] = [
			    					{ qty: 1000, price: 10 },
			    					{ qty: 1500, price: 10 },
			    					{ qty: 2000, price: 10 },
		    					...
		    					$scope.options.combination.options[k1][k2][k3]['qty_breaks'].sort( function( a , b) { return a.qty - b.qty });
		    				}
		    				index++;
						} 
					})
    ...
    $scope.kita_sort_qty_breaks = function( k1 , k2 , k3 ) {
    	$scope.options.combination.options[k1][k2][k3]['qty_breaks'].sort( function( a , b) { return a.qty - b.qty });
    }
    $scope.kita_add_qty_breaks = function( k1 , k2 , k3 ) {
    	$scope.options.combination.options[k1][k2][k3]['qty_breaks'].push(
    		{ qty: 0, price: 0 }
    	)
    }
    $scope.kita_remove_qty_breaks = function(index , k1 , k2 , k3 ) {
    	if( $scope.options.combination.options[k1][k2][k3]['qty_breaks'].length <= 1 ) return;
    	$scope.options.combination.options[k1][k2][k3]['qty_breaks'].splice( index , 1 );
    }
    $scope.show_quantity_break_combination = function(area, size, material) {
    	$scope.options.combination.options[area][size][material].show = !$scope.options.combination.options[area][size][material].show;
    }
    $scope.set_default_combination = function( area, size, material , all = true ) {
    	let _default = $scope.options.combination.default;
    	if(all) {
    		var index = 1;
    		angular.forEach( $scope.options.combination.options , function( area , k1 ) {
    			angular.forEach( area , function( size , k2 ) {
    				angular.forEach( size , function( material , k3 ) {
						if($scope.options.combination.options[k1][k2][k3].coming_soon != 'on') {
							$scope.options.combination.options[k1][k2][k3].price = _default.price + index;
		    				$scope.options.combination.options[k1][k2][k3].qty = _default.qty + index;
		    				$scope.options.combination.options[k1][k2][k3].index = index;
		    				index++;
						} 
    				})
    		...
    		if(angular.isDefined( $scope.options.combination.default )) {
	    		if($scope.options.combination.options[area][size][material].default) {
			    	$scope.options.combination.options[area][size][material].price = _default.price;
			    	$scope.options.combination.options[area][size][material].qty = _default.qty;
		    	} else {
		    		$scope.options.combination.options[area][size][material].price = 0;
			    	$scope.options.combination.options[area][size][material].qty = 0;
		    	}
		    	$scope.options.combination.options[area][size][material].default = !$scope.options.combination.options[area][size][material].default;
		    }
    	}
    	
    }
    $scope.show_options_combination = function() {
    	if($scope.options.combination.show || $scope.options.combination.show == 'on' ) {
    		$scope.options.combination.show = false;
    	} else {
    		$scope.options.combination.show = 'on';
    	}
    }
    $scope.importCombination = function(){
                ...
                reader.onload = function(event){
                    if (event.target.readyState === 2) {
                        ...
                        $timeout(function() {
                        	...
                        	// angular.forEach( allTextLines , function( row , k1 ) {
                        	// 	angular.forEach( row , function( col , k2 ) {
	                        // 		if( col != '' && k1 == 0 ) {
	                        // 			shapes.push(col);
	                        // 		}
	                        // 	})
                        	// 	var _row =row.split(',');
                        	// 	combinations.push(_row);
                        	// })
                        	var shape = '';
                    		var size = '';
                    		var material = '';
                        	angular.forEach( allTextLines , function( row , k ) {
                        		allTextLines[k] = row.split(',');
                        		angular.forEach( allTextLines[k] , function( col , k1 ) {
                        		 	if( allTextLines[0][k1].length > 2 ) {
                        		 		shape = allTextLines[0][k1].replace(/['"]+/g, '');
                        		 		if( shape.toLowerCase() == 'circle' || shape.toLowerCase() == 'square' ) { shape = 'Square + Circle'; }
                        		 		if( shape.toLowerCase() == 'rectangle' || shape.toLowerCase() == 'oval' ) { shape = 'Rectangle + Oval'; }
                        		 		if( angular.isUndefined(combinations[shape]) ) {
                        		 			combinations[shape] = [];
                        		 		}
                        		 	}
                        		 	if( allTextLines[1][k1].length > 2 && k1 > 0 ) {
                        		 		size = allTextLines[1][k1].replace(/['"]+/g, '');
                        		 		if( angular.isDefined(combinations[shape]) && angular.isUndefined(combinations[shape][size]) ) {
                        		 			combinations[shape][size] = [];
                        		 		}
                        		 	}
                        		 	if( allTextLines[2][k1].length > 2 && k1 > 0) {
                        		 		material = allTextLines[2][k1].replace(/['"]+/g, '');
                        		 		if( angular.isDefined(combinations[shape]) && angular.isDefined(combinations[shape][size]) && angular.isUndefined(combinations[shape][size][material]) ) {
                        		 			combinations[shape][size][material] = {
                        		 				qty: 0,
                        		 				price: 0,
                        		 				qty_breaks: [],
                        		 			};
                        		 		} else {
	                        		 		combinations[shape][size][material]['show'] = false;
	                        		 		if( k > 2 ) {
	                        		 			var qty = parseInt(allTextLines[k][0].replace(/['"]+/g, ''));
	                        		 			var price = parseInt(allTextLines[k][k1].replace(/['"]+/g, ''));
	                        		 			combinations[shape][size][material].qty_breaks.push( { qty: qty , price: price} );
	                        		 			combinations[shape][size][material]['price'] = parseInt(allTextLines[3][k1].replace(/['"]+/g, ''));;
	                        		 			combinations[shape][size][material]['qty'] = parseInt(allTextLines[3][0].replace(/['"]+/g, ''));;
	                        		 		}
                        		 		}
                        		 	}
                        		 })                     	
								...
								angular.forEach( area , function( size , k2 ) {
									angular.forEach( size , function( material , k3 ) {
										if($scope.options.combination.options[k1][k2][k3].coming_soon != 'on') {
						    				...
						    				if( angular.isDefined(combinations[k1]) && angular.isDefined(combinations[k1][k2]) && angular.isDefined(combinations[k1][k2][k3]) ) {
					    						$scope.options.combination.options[k1][k2][k3]['qty_breaks'] = combinations[k1][k2][k3].qty_breaks;
					    						$scope.options.combination.options[k1][k2][k3]['price'] = combinations[k1][k2][k3].price;
					    						$scope.options.combination.options[k1][k2][k3]['qty'] = combinations[k1][k2][k3].qty;
					    					}
						    				$scope.options.combination.options[k1][k2][k3].index = index;
						    				index++;
										} 
                        ...
                        jQuery('.nbp-loading-wrap').removeClass('nbp-show');
                        jQuery('.result-import').html('Import Success');
                        alert('Uploaded, please check that the value is correct now!');
                    };
                };
    ...
    $scope.custom_upload_file = function(  e , file , type) {
    	var name = jQuery(e).data('file-name');
    	var fieldIndex = jQuery(e).data('field-index');
    	var opIndex = jQuery(e).data('op-index');
    	var formData = new FormData;
    	formData.append('action', 'nbd_upload_file_guideline');
        formData.append('file', file);
        formData.append('nonce', nbnonce);
    	...
    	jQuery.ajax({
        	...
        	$scope.options['fields'][fieldIndex]['general']['attributes']['options'][opIndex][type] = {};
            $scope.options['fields'][fieldIndex]['general']['attributes']['options'][opIndex][type] = {
            	name: data.name,
            	link: data.link,
	        };
	        if( type == 'guideline_ai') {
	        	jQuery(e).parents('.wrap-guideline-ai').find('input.guideline-ai-name').attr('value' , data.name);
	        	jQuery(e).attr('value' , data.link);
	        	jQuery(e).parents('.wrap-guideline-ai').find('.file-result').html(data.name);
	        }
	        if( type == 'guideline_pdf') {
	        	jQuery(e).parents('.wrap-guideline-pdf').find('input.guideline-pdf-name').attr('value' , data.name);
	        	jQuery(e).attr('value' , data.link);
	        	jQuery(e).parents('.wrap-guideline-pdf').find('.file-result').html(data.name);
	        }
        });
    };
    $scope.custom_remove_file = function( fieldIndex , opIndex , type ) {
    	if( angular.isDefined($scope.options['fields'][fieldIndex]['general']['attributes']['options'][opIndex][type]) ) {
    		$scope.options['fields'][fieldIndex]['general']['attributes']['options'][opIndex][type]['name'] = '';
    		$scope.options['fields'][fieldIndex]['general']['attributes']['options'][opIndex][type]['link'] = '';
    	}
    }
