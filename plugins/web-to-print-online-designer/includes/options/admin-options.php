    ...
    class NBD_ADMIN_PRINTING_OPTIONS {
        ...
        public function ajax(){
            $ajax_events = array(
                ...
                'nbd_upload_file_guideline'     => true, // custom kitalabel
            );
        ...
        // custom kitalabel
        public function nbd_upload_file_guideline(){
            ...
            if( $filename ) {
                $new_name   = time() . '_' . sanitize_file_name( $filename ).'.'.$ext;
            } else {
                $new_name   = time() . '_' . sanitize_file_name( $_FILES['file']["name"] );
            }
            $path_dir   = NBDESIGNER_DATA_DIR . '/guidelines'; 
            ...
            if( move_uploaded_file( $_FILES['file']["tmp_name"], $path_file ) ){
                ...
                $result['link'] = Nbdesigner_IO::wp_convert_path_to_url( $path_file );
                $result['flag'] = 1;
            }
        ...
        public function build_options( $options = null ){
            if( is_null( $options ) ){
                $options = array(
                    ...
                    'combination'               => array(), // custom kitalabel
                );
            }
